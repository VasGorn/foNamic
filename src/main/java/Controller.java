	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action != null) {
			...
			case "upload":
				...
				try {
					reader = new CSVParser(in, CSVFormat.DEFAULT);
					rowList = reader.getRecords();
					
					List<Double> lyData = new LinkedList<>();
					String[] row;
					
					for(CSVRecord item: rowList) {
						String sItem = item.get(0);
						double numItem = StrToNumber.getDouble(sItem);
						lyData.add(numItem);
